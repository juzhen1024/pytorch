...
for name in list(globals().keys()):
    if name.startswith("DynamicShapesCodegenCpuTests") and name != "DynamicShapesCodegenCpuTests":
        _apply_test_failures(globals()[name], test_failures)


if not TEST_WITH_ASAN:
    _copy_template_tests(DynamicShapesCodegenCommonTemplate, DynamicShapesCodegenGPUTests)
    ...
    for name in list(globals().keys()):
        if name.startswith("DynamicShapesCodegenGPUTests") and name != "DynamicShapesCodegenGPUTests":
            _apply_test_failures(globals()[name], test_failures)

    for name in list(globals().keys()):
        cls = globals()[name]
        if (
            name.startswith("DynamicShapesCodegenGPUTestsCUDA")
            and hasattr(cls, "test_randint_distribution_dynamic_shapes")
        ):
